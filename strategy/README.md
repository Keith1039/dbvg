...
Each function can "hook-in" to the internal systems and allow the `Strategy` to be recognized when it comes time
to generate data. Validation for all 3 is mostly the same, ensuring that an existing Strategy doesn't exist internally 
i.e. no overwriting existing entries. They also ensure that the given columnType specified is supported and that the new entry
